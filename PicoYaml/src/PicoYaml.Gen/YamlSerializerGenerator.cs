...
public sealed class YamlSerializerGenerator : IIncrementalGenerator
{
    ...
    private static string GenRefStruct(TypeInfo t)
    {
        ...
        // Serializer
        s.Append("file static class "); s.Append(t.Name); s.AppendLine("YamlSer {");
        ...
        foreach (var p in t.Properties)
        {
            ...
            s.AppendLine("\"u8);");
            s.Append("        yw.WriteNumber("); s.Append($"v.{p.Name}"); s.AppendLine(");");
        }
        s.AppendLine("        yw.WriteEndMapping();");
        s.AppendLine("    } }");
        s.AppendLine();
        ...
        s.Append("        YamlSerializer.Register<"); s.Append(typeRef); s.AppendLine(">(");
        s.Append("            "); s.Append(t.Name); s.AppendLine("YamlSer.Serialize);");
        s.AppendLine("    } }");
        return s.ToString();
    }
