...
public static partial class YamlSerializer
{
    ...
    { _serializers[typeof(T)] = serializer; _deserializers[typeof(T)] = deserializer; }

    public static byte[] SerializeToUtf8Bytes<T>(T value)
    {
        if (_serializers.TryGetValue(typeof(T), out var s))
        { var w = new ArrayBufferWriter<byte>(); ((ISerializer<T>)s).Serialize(w, value); return w.WrittenSpan.ToArray(); }
        ThrowNoSer<T>(); return default;
    }
    ...
    {
        if (_deserializers.TryGetValue(typeof(T), out var d)) return ((IDeserializer<T>)d).Deserialize(data);
        ThrowNoSer<T>(); return default;
    }

    [System.Diagnostics.CodeAnalysis.DoesNotReturn]
    private static void ThrowNoSer<T>() => throw new InvalidOperationException(
