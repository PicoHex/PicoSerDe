...
public class IniReaderTests
{
    ...
    {
        var data = "[Server]"u8;
        bool ok; bool sectionOk;
        { var r = new IniReader(data); ok = r.Read(); sectionOk = r.SectionName.SequenceEqual("Server"u8); }
    ...
    public async Task Read_SemicolonComment_ReturnsComment()
    {
        ...
        IniTokenType tt; bool ctOk;
        { var r = new IniReader(data); r.Read(); tt = r.TokenType; ctOk = r.CommentText.SequenceEqual(" this is a comment"u8); }
    ...
    public async Task Read_HashComment_ReturnsComment()
    {
        ...
        IniTokenType tt;
        { var r = new IniReader(data); r.Read(); tt = r.TokenType; }
        await Assert.That(tt).IsEqualTo(IniTokenType.Comment);
    }
    ...
    {
        bool ok;
        { var r = new IniReader(default(ReadOnlySpan<byte>)); ok = r.Read(); }
    ...
    public async Task Read_QuotedValue_Unquotes()
    {
        ...
        bool valOk;
        { var r = new IniReader(data); r.Read(); valOk = r.ValueSpan.SequenceEqual("hello world"u8); }
        await Assert.That(valOk).IsTrue();
    }
    ...
    public async Task TryGetInt32_ParsesInteger()
    {
        ...
        bool ok; int v;
        { var r = new IniReader(data); r.Read(); ok = r.TryGetInt32(out v); }
        await Assert.That(ok).IsTrue();
    ...
    public async Task TryGetBool_Works()
    {
        ...
        bool ok1, ok2, v1, v2;
        { var r = new IniReader(d1); r.Read(); ok1 = r.TryGetBool(out v1); }
        { var r = new IniReader(d2); r.Read(); ok2 = r.TryGetBool(out v2); }
    ...
    public async Task SectionNameEquals_CaseInsensitive()
    {
        var data = "[Server]"u8;
        bool eq1, eq2, eq3;
        { var r = new IniReader(data); r.Read(); eq1 = r.SectionNameEquals("SERVER"u8); eq2 = r.SectionNameEquals("server"u8); eq3 = r.SectionNameEquals("Server"u8); }
    ...
    {
        var data = "; Config\r\n[Server]\r\nHost = localhost\r\nPort = 8080"u8;
        bool cOk = false, sOk = false, hOk = false, pOk = false; int step = 0;
        {
            var r = new IniReader(data);
            while (r.Read())
            {
                if (step == 0) { cOk = r.TokenType == IniTokenType.Comment; }
                else if (step == 1) { sOk = r.TokenType == IniTokenType.SectionStart && r.SectionName.SequenceEqual("Server"u8); }
                else if (step == 2) { hOk = r.Key.SequenceEqual("Host"u8) && r.ValueSpan.SequenceEqual("localhost"u8); }
                else if (step == 3) { pOk = r.Key.SequenceEqual("Port"u8) && r.ValueSpan.SequenceEqual("8080"u8); }
                step++;
            }
        }
    ...
    public async Task SequenceReader_Parses()
    {
        ...
        bool ok; bool sOk;
        { var r = new IniReader(seq); ok = r.Read(); sOk = r.SectionName.SequenceEqual("Test"u8); }
        await Assert.That(ok).IsTrue();
        await Assert.That(sOk).IsTrue();
    }
    ...
    public async Task MalformedSection_ThrowsFormatException()
    {
        ...
        bool threw = false;
        try { var r = new IniReader(data); r.Read(); } catch (FormatException) { threw = true; }
