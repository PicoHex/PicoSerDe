...
public sealed class MsgPackSerializerGenerator : IIncrementalGenerator
{
    ...
    static void EmitMpReadValue(StringBuilder s, PropertyInfo prop)
    {
        switch (prop.TypeKind)
        {
            case "string": s.Append("Encoding.UTF8.GetString(reader.GetStringRaw())"); break;
            case "int32": s.Append("reader.TryGetInt32(out var __iv) ? __iv : 0"); break;
            case "int64": s.Append("reader.TryGetInt64(out var __lv) ? __lv : 0"); break;
            case "float64": case "float32": s.Append("reader.TryGetFloat64(out var __dv) ? __dv : 0"); break;
            case "boolean": s.Append("reader.TryGetBool(out var __bv) ? __bv : false"); break;
            default: s.Append("default"); break;
        }
    }
}
