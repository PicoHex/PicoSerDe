...
public sealed class MsgPackSerializerGenerator : IIncrementalGenerator
{
    ...
    static string GenRefStruct(TypeInfo t)
    {
        ...
        {
            s.Append("using "); s.Append(t.Namespace); s.AppendLine(";");
        }
        s.AppendLine();

        // Serializer
        s.Append("file static class "); s.Append(t.Name); s.AppendLine("MsgPackSer {");
        s.Append("    public static void Serialize(IBufferWriter<byte> writer, "); s.Append(t.Name); s.AppendLine(" v) {");
        s.Append("        var mw = new MsgPackWriter(writer); mw.WriteStartObject("); s.Append(t.Properties.Length); s.AppendLine(");");
        foreach (var p in t.Properties)
        {
            s.Append("        mw.WriteInt32("); s.Append($"v.{p.Name}"); s.AppendLine(");");
        }
        s.AppendLine("        mw.WriteEndObject(); } }");
        s.AppendLine();
        ...
        s.Append("file static class "); s.Append(t.Name); s.AppendLine("SerDeRegistration {");
        s.AppendLine("    [ModuleInitializer]");
        s.AppendLine("    internal static void Register() {");
        s.Append("        MsgPackSerializer.Register<"); s.Append(typeRef); s.AppendLine(">(");
        s.Append("            "); s.Append(t.Name); s.AppendLine("MsgPackSer.Serialize);");
        s.AppendLine("    } }");
        return s.ToString();
    }
