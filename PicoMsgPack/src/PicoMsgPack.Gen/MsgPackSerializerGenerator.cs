...
public sealed class MsgPackSerializerGenerator : IIncrementalGenerator
{
    ...
    static void GenerateAll(SourceProductionContext spc, ImmutableArray<TypeInfo> types)
    {
        ...
        foreach (var kv in nestedTypes)
        {
            ...
            if (!hintNames.Add(hintName))
                continue;
            var polyTarget = types.FirstOrDefault(t =>
            ...
            spc.AddSource(
                ...
                SourceText.From(
                    polyTarget.FullyQualifiedName is not null
                        ? GenPolyInner(fullName, sn, polyTarget)
                        : GenInner(fullName, sn, props),
                    Encoding.UTF8
                )
            );
        }
    ...
    /// nested value goes through the discriminator-aware serializer and
    /// deserializer instead of an empty base instance.
    /// </summary>
    static string GenPolyInner(string fqn, string shortName, TypeInfo polyType)
    {
        ...
        var poly = MsgPackPolyPrefix(polyType);
        s.Append("internal static class ");
        s.Append(shortName);
        ...
        s.Append("        => ");
        s.Append(poly);
        s.AppendLine("MsgPackPolySer.Serialize(mw.Buffer, v, mw.Options);");
        s.AppendLine();
        s.Append("    internal static ");
        s.Append(clean);
        s.AppendLine(" Deserialize(ref MsgPackReader reader)");
        s.Append("        => ");
        s.Append(poly);
        s.AppendLine("MsgPackPolyDes.DeserializeCore(ref reader, null);");
        s.AppendLine("}");
        return s.ToString();
    }
        ...
        switch (p.TypeKind)
        {
            ...
            case "object":
                if (p.NestedProperties.Length == 0)
                {
                    if (p.IsRecursiveRef)
                    {
                        // Cycle-truncated member: the seeded helper serializes the
                        // target type (null here would silently drop the value).
                        var recSn = PicoSerDe.Gen.GenInfrastructure.InnerClassName(
                            "MsgPackInner",
                            p.TypeFullName!
                        );
                        s.Append(ind);
        ...
        switch (p.TypeKind)
        {
            ...
            case "object":
                if (p.NestedProperties.Length == 0)
                {
                    if (p.IsRecursiveRef)
                    {
                        // Cycle-truncated member: read via the seeded helper.
                        var recSn = PicoSerDe.Gen.GenInfrastructure.InnerClassName(
                            "MsgPackInner",
                            p.TypeFullName!
                        );
    ...
    /// deserializer: the recursion helper routes nested values through the
    /// discriminator dispatch (abstract bases cannot be instantiated).
    /// </summary>
    static string MsgPackPolyPrefix(TypeInfo type) =>
    ...
    static string GenPoly(TypeInfo type, Dictionary<string, TypeInfo> typeMap)
    {
        ...
        // ── Serializer ──
        s.Append("internal static class ");
        s.Append(MsgPackPolyPrefix(type));
        s.AppendLine("MsgPackPolySer {");
        s.Append("    public static void Serialize(IBufferWriter<byte> writer, ");
        s.Append(type.Name);
        s.AppendLine(" value, global::PicoSerDe.Core.SerOptions? options) {");
        s.AppendLine(
        ...
        s.AppendLine("        return DeserializeCore(ref reader, options);");
        s.AppendLine("    }");
        s.AppendLine();
        s.Append("    public static ");
        s.Append(type.Name);
        s.AppendLine(
            " DeserializeCore(ref MsgPackReader reader, global::PicoSerDe.Core.SerOptions? options) {"
        );
        ...
        s.Append("            ");
        s.Append(MsgPackPolyPrefix(type));
        s.AppendLine("MsgPackPolySer.Serialize,");
        s.Append("            ");
        s.Append(MsgPackPolyPrefix(type));
        s.AppendLine("MsgPackPolyDes.Deserialize);");
        s.AppendLine("    }");
        s.AppendLine("}");
