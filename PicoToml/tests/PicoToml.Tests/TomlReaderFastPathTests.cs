...
public class TomlReaderFastPathTests
{
    // TOML inline arrays appear as values: key = [1, 2, 3]
    // After Read()→PropertyName("key") and Read()→ArrayStart,
    // the reader position is past '[' and the fast path can scan elements.

    [Test]
    public async Task TryReadInt32ArrayFast_Basic()
    {
        var toml = "arr = [1, 42, -7, 0]"u8.ToArray();
        var reader = new TomlReader(toml);
        reader.Read(); // PropertyName "arr" — advances past '['
    ...
    {
        var toml = "arr = [1, -5, 9223372036854775807]"u8.ToArray();
        var reader = new TomlReader(toml);
        reader.Read();
        reader.Read();
    ...
    {
        var toml = "arr = [true, false, true]"u8.ToArray();
        var reader = new TomlReader(toml);
        reader.Read();
        reader.Read();
