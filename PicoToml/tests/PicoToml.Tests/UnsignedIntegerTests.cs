...
public class TomlUnsignedFamilyDto
{
...
public class TomlDictDto
{
    public Dictionary<string, ulong> Map { get; set; } = new();
}

/// <summary>
/// PicoToml must support the full C# integer family (uint/ushort/short/byte/
...
public class UnsignedIntegerTests
{
    ...
    {
        var dto = new TomlUnsignedFamilyDto
        {
            U = uint.MaxValue,
            S = short.MinValue,
            US = ushort.MaxValue,
        ...
        var back = TomlSerializer.Deserialize<TomlUnsignedFamilyDto>(
            TomlSerializer.SerializeToUtf8Bytes(dto)
        );
    ...
    {
        var back = TomlSerializer.Deserialize<List<ulong>>(
            TomlSerializer.SerializeToUtf8Bytes(new List<ulong> { ulong.MaxValue, 7UL })
        );
    ...
    {
        var dto = new TomlDictDto
        {
            Map = new Dictionary<string, ulong> { ["a"] = 1UL, ["b"] = ulong.MaxValue },
        };
        var back = TomlSerializer.Deserialize<TomlDictDto>(
            TomlSerializer.SerializeToUtf8Bytes(dto)
        );
    ...
    public async Task Deserialize_NegativeIntoUnsigned_Throws()
    {
        await Assert
            .That(
                ThrowsFormat(() => TomlSerializer.Deserialize<TomlUnsignedFamilyDto>("U = -1\n"u8))
            )
            .IsTrue();
        await Assert
            .That(
                ThrowsFormat(() => TomlSerializer.Deserialize<TomlUnsignedFamilyDto>("L = -1\n"u8))
            )
            .IsTrue();
    }
    ...
    public async Task Deserialize_OutOfRange_Throws()
    {
        await Assert
            .That(
                ThrowsFormat(() =>
                    TomlSerializer.Deserialize<TomlUnsignedFamilyDto>("U = 4294967296\n"u8)
                )
            )
            .IsTrue();
        await Assert
            .That(
                ThrowsFormat(() =>
                    TomlSerializer.Deserialize<TomlUnsignedFamilyDto>(
