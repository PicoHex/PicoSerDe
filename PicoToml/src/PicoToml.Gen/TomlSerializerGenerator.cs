...
public sealed class TomlSerializerGenerator : IIncrementalGenerator
{
    ...
    private static string GenRefStruct(TypeInfo t)
    {
        ...
        // Serializer
        s.Append("file static class "); s.Append(t.Name); s.AppendLine("TomlSer {");
        s.Append("    public static void Serialize(IBufferWriter<byte> w, "); s.Append(t.Name); s.AppendLine(" v) {");
        s.AppendLine("        var tw = new TomlWriter(w);");
        foreach (var p in t.Properties)
        {
            s.Append("        tw.WriteKeyValue(\"");
            s.Append(PicoSerDe.Gen.GenInfrastructure.EscapeCSharpString(p.JsonName));
            s.Append("\", "); s.Append($"v.{p.Name}"); s.AppendLine(");");
        }
        s.AppendLine("    } }");
        s.AppendLine();
        ...
        s.Append("        TomlSerializer.Register<"); s.Append(typeRef); s.AppendLine(">(");
        s.Append("            "); s.Append(t.Name); s.AppendLine("TomlSer.Serialize);");
        s.AppendLine("    } }");
        return s.ToString();
    }
