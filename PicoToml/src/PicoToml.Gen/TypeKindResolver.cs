...
internal static class TypeKindResolver
{
    public static (string? Kind, bool IsNullable, ITypeSymbol? InnerType) Resolve(ITypeSymbol type)
    {
        ...
        string? kk = type.SpecialType switch
        {
            ...
            SpecialType.System_Boolean => "boolean", SpecialType.System_DateTime => "datetime",
            SpecialType.System_Decimal => "decimal", _ => null
        };
        if (kk is null && type is INamedTypeSymbol { TypeKind: TypeKind.Enum }) kk = "enum";
        if (kk is null && type is INamedTypeSymbol { TypeKind: TypeKind.Class or TypeKind.Struct } o)
        { foreach (var m in o.GetMembers()) if (m is IPropertySymbol { DeclaredAccessibility: Accessibility.Public, IsStatic: false } p && p.GetMethod is not null) return ("object", false, null); }
    ...
    public static string MapTypeName(string k, ITypeSymbol t) => k switch
    {
        ...
        "boolean" => "bool", "datetime" => "System.DateTime", "decimal" => "decimal",
        "enum" => t.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat),
        "object" => t.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat), _ => "object"
    };
}
