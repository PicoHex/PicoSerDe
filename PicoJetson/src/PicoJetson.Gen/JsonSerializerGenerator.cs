...
public sealed class JsonSerializerGenerator : IIncrementalGenerator
{
    ...
    )
    {
        ...
        // Only emit namespace for non-generic type names (generic names contain '<')
        // because LastIndexOf('.') would find dots inside type parameters.
    ...
    private static string GeneratePolyHelper(TypeInfo type, string safeName, string cleanName)
    {
        var hasCtor = !type.CtorParams.IsDefaultOrEmpty && type.CtorParams.Length > 0;
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/>");
        ...
        if (lastDot > 0)
        {
            var ns = cleanName.Substring(0, lastDot);
            sb.AppendLine();
            sb.Append("namespace ");
            sb.Append(ns);
            sb.AppendLine(";");
        ...
        sb.Append("file static class ");
        sb.Append(safeName);
        sb.AppendLine("JsonPoly");
        sb.AppendLine("{");

        // ── SerializeProps ──
        sb.Append("    internal static void SerializeProps(ref JsonWriter jw, ");
        sb.Append(cleanName);
        sb.AppendLine(" value)");
        sb.AppendLine("    {");
        foreach (var prop in type.Properties)
        {
            var name = prop.JsonName ?? prop.Name;
            sb.Append("        jw.WritePropertyName(\"");
            sb.Append(EscapeCSharpString(name));
            sb.AppendLine("\"u8);");
            EmitSerializeProperty(sb, prop, "value." + prop.Name, "        ");
        }
        sb.AppendLine("    }");
        ...
        for (int i = 0; i < type.Properties.Length; i++)
        {
            var prop = type.Properties[i];
            var kw = i == 0 ? "if" : "else if";
            var pn = prop.JsonName ?? prop.Name;
            sb.Append("            ");
            sb.Append(kw);
            sb.Append(" (TextHelpers.Eq(__n, \"");
            sb.Append(EscapeCSharpString(pn));
