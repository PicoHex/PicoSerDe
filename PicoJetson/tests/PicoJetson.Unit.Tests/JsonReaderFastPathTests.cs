...
public class JsonReaderFastPathTests
{
    [Test]
    public async Task TryReadInt32ArrayFast_Basic()
    {
        var json = "[1, 42, -7, 0]"u8.ToArray();
    ...
    {
        var json = "[1, -5, 9223372036854775807]"u8.ToArray();
        var reader = new JsonReader(json);
        reader.Read();
    ...
    {
        var json = "[true, false, true]"u8.ToArray();
        var reader = new JsonReader(json);
        reader.Read();
    ...
    [Test]
    public async Task TryReadInt32ArrayFast_WithSpaces()
    {
        var json = "[ 1 , -42 , 7 ]"u8.ToArray();
        var reader = new JsonReader(json);
        reader.Read();
        var list = new List<int>();
        var buf = new int[3];
